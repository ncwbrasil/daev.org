				...
				<div class='conteudo'>
					<?php 
						if($url == '' && $categoria == ''){
							...
							WHERE nt_status = :nt_status AND nt_data <= :nt_data AND nt_hora <= nt_hora
							ORDER BY nt_id DESC";
							$stmt = $PDO->prepare($sql);
							$stmt->bindValue(':nt_status', 1);
							$stmt->bindValue(':nt_data', date('Y-m-d'));
							$stmt->bindValue(':nt_hora', date('H:i:s'));			
							$stmt->execute();
								$i = 0; 
								$c = 10; 
								$p = 1; 
								while($result = $stmt->fetch()){
									...
									$nt_subtitulo 	= $result['nt_subtitulo']; 
									$nt_imagem 	= $result['cni_foto']; 
									$data       = implode("/", array_reverse(explode("-", $result['nt_data'])));
						...
						if($url != ''){
							echo"
								...
								<div class='imagem'>
									<img src='webapp/uploads/noticias/$nt_imagem' style='width:100%;'>
									<i>$nt_ttl_imagem</i>
								</div><br>
